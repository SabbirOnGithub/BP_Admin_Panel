...
import {
	axiosWithoutToken,
	axiosWithToken,
	axiosWithTokenAndMultipartData,
} from "../../helpers/axios";
...
const listHomePageFunctionAreaDetails = () => async (dispatch) => {
	try {
		...
		if (data.status === true) {
			dispatch({
				type: HOME_PAGE_FUNCTION_AREA_DETAIL_LIST_SUCCESS,
				payload: data.data ? data.data?.reverse() : [],
			});
		} else {
			dispatch({
				type: HOME_PAGE_FUNCTION_AREA_DETAIL_LIST_FAIL,
...
const detailsHomePageFunctionAreaDetail = (id) => async (dispatch) => {
	try {
		...
		const {data} = await axiosWithoutToken.get(
			"/HomePageFunctionAreaDetails/" + id
		);
		dispatch({
			type: HOME_PAGE_FUNCTION_AREA_DETAIL_DETAILS_SUCCESS,
			payload: data,
		});
	} catch (error) {
		dispatch({
			type: HOME_PAGE_FUNCTION_AREA_DETAIL_DETAILS_FAIL,
			payload: error.message,
		});
	}
};
...
const saveHomePageFunctionAreaDetail = (item, id) => async (dispatch) => {
	try {
		dispatch({
			type: HOME_PAGE_FUNCTION_AREA_DETAIL_SAVE_REQUEST,
			payload: item,
		});
		if (!id) {
			console.log("create");
			...
			const {data} = await axiosWithTokenAndMultipartData.post(
				"/HomePageFunctionAreaDetails/Create",
				item
			);
			// console.log(data)
			if (data.status === true) {
				dispatch({
					type: HOME_PAGE_FUNCTION_AREA_DETAIL_SAVE_SUCCESS,
					payload: data,
				});
			} else {
				dispatch({
					type: HOME_PAGE_FUNCTION_AREA_DETAIL_SAVE_FAIL,
					payload: data.message,
				});
			}
		} else {
			const {data} = await axiosWithTokenAndMultipartData.put(
				"/HomePageFunctionAreaDetails/Update",
				item
			);
			console.log(data);
			if (data.status === true) {
...
const deleteHomePageFunctionAreaDetail = (id) => async (dispatch, getState) => {
	try {
		...
		if (data.status === true) {
			dispatch({
				type: HOME_PAGE_FUNCTION_AREA_DETAIL_DELETE_SUCCESS,
				payload: data,
				success: true,
			});
		} else {
			dispatch({
				type: HOME_PAGE_FUNCTION_AREA_DETAIL_DELETE_FAIL,
				payload: data.message,
			});
		}
	} catch (error) {
...
export {
	listHomePageFunctionAreaDetails,
	detailsHomePageFunctionAreaDetail,
	saveHomePageFunctionAreaDetail,
	deleteHomePageFunctionAreaDetail,
};
