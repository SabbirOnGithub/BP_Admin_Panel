	...
	if (item) {
		...
		try {
			...
			if (data.status === true) {
				dispatch({
					...
					payload: data?.data?.item1 ? data.data : [],
				});
		...
		try {
			...
			const {data} = await axiosWithoutToken.get(
				`${BASE_API_URL}/User/searchAll`
			);
			if (data.status === true) {
				dispatch({
					type: USER_LIST_SUCCESS,
					payload: data?.data?.item1 ? data.data.item1?.reverse() : [],
				});
				// console.log(data)
			} else {
				dispatch({type: USER_LIST_FAIL, payload: data.message});
...
const listAcceptClientUsers = () => async (dispatch) => {
	try {
		...
		const {data} = await axiosWithoutToken.get(
			`${BASE_API_URL}/User/UserDropdownList`
		);
		if (data.status === true) {
...
const changePassword = (item) => async (dispatch) => {
	// console.log(item)
	try {
		dispatch({type: USER_PASSWORD_CHANGE_REQUEST, payload: item});
		console.log("Change Password");
		//eslint-disable-next-line

		const {data} = await axiosWithToken.post("/User/ChangePassword", item);
		console.log(data);
		if (data.status === true) {
			dispatch({type: USER_PASSWORD_CHANGE_SUCCESS, payload: data});
		} else {
			dispatch({type: USER_PASSWORD_CHANGE_FAIL, payload: data.message});
		}
		// console.log(data)
		return data;
	} catch (error) {
...
const signin = (email, password) => async (dispatch) => {
	...
	try {
		const {data} = await axiosWithoutToken.post(`${BASE_API_URL}/Auth/Login`, {
			username,
			password,
		});
...
const deActivateUser = (id) => async (dispatch) => {
	try {
		...
		if (data.status === true) {
			Cookie.remove("userInfo");
			Cookie.remove("userToken");
			dispatch({type: USER_DEACTIVATE_SUCCESS, payload: data, success: true});
		} else {
			dispatch({type: USER_DEACTIVATE_FAIL, payload: data.message});
		}
	} catch (error) {
		dispatch({type: USER_DEACTIVATE_FAIL, payload: error.message});
	}
};
