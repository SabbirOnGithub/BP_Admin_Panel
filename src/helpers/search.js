...
const searchNameByIdFromArray = (arr, id) => {
  ...
  // console.log(id)
  const name =
    Array.isArray(arr) && typeof id !== "undefined"
      ? arr.find((item) => item.id === Number(id))?.name
      : id
      ? id
      : "no data found";
  return name;
...
const getPermissions = (currentPathName) => {
  ...
  const permission = Array.isArray(roleResource)
    ? roleResource.filter(
        (item) =>
          item.systemName != null &&
          item.systemName?.toUpperCase() === currentPathName?.toUpperCase()
      )[0]
    : {};
  return permission;
};
...
const getFilterDataByUser = (dataOfArray, userInfo) => {
  ...
  if (dataOfArray && userInfo) {
    ...
    const tableDataFilterByUser =
      userInfo?.userRole === 1 || userInfo?.userRole === 2
        ? dataOfArray
        : userInfo?.userRole === 3
        ? []
        : dataOfArray?.filter((item) => item?.email === userInfo?.email);
