...
import SoftwareTrainingSummaryForm from "./SoftwareTrainingSummaryForm";
...
// redux actions
import { listSoftwareTrainingSummarys, saveSoftwareTrainingSummary, deleteSoftwareTrainingSummary } from '../../../redux/actions/softwareTrainingSummaryActions';
...
export default function SoftwareTrainingSummaryScreen() {
    // permission get
    const {
        permission,
        setPermission,
    ...
    const softwareTrainingSummaryList = useSelector(state => state.softwareTrainingSummaryList);
    //eslint-disable-next-line
    const { softwareTrainingSummarys, loading, error } = softwareTrainingSummaryList;
    const softwareTrainingSummarySave = useSelector(state => state.softwareTrainingSummarySave);
    //eslint-disable-next-line
    const { loading: loadingSave, success: successSave, error: errorSave } = softwareTrainingSummarySave;
    const softwareTrainingSummaryDelete = useSelector(state => state.softwareTrainingSummaryDelete);
    //eslint-disable-next-line
    const { loading: loadingDelete, success: successDelete, error: errorDelete } = softwareTrainingSummaryDelete;
    ...
    } = useTable(softwareTrainingSummarys, headCells, filterFn);

    const dispatch = useDispatch();

    // add/update promise
    const saveItem = (item) => new Promise((resolve, reject) => {
        dispatch(saveSoftwareTrainingSummary(item));
        resolve();
    })

    // delete promise
    const deleteItem = (id) => new Promise((resolve, reject) => {
        dispatch(deleteSoftwareTrainingSummary(id));
        resolve();
    })
    ...
    useEffect(() => {
        try {
            ...
            if (recievedPermission?.readOperation) {
                ...
                dispatch(listSoftwareTrainingSummarys());
        ...
        <>
            {
                (loadingRoleResource || loading || loadingSoftwares || loadingTrainingTypes || loadingSave || loadingDelete) ? <Loading /> :
                    (
                        softwareTrainingSummarys.length >= 0 &&
                        <>
                            <PageTitle title="Software Training Summarys" />

                            <Grid container spacing={4}>
                                <Grid item xs={12}>
                                    <Widget
                                        title="Software Training Summary List Table"
                                        upperTitle
                                        noBodyPadding
                                    ...
                                    >
                                        ...
                                        <Popup
                                            title="Software Training Summary Form"
                                            openPopup={openPopup}
                                            setOpenPopup={setOpenPopup}
                                        >
                                            <SoftwareTrainingSummaryForm
