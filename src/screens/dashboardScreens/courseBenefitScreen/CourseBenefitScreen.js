...
import CourseBenefitForm from "./CourseBenefitForm";
...
// redux actions
import { listCourseBenefits, saveCourseBenefit, deleteCourseBenefit } from '../../../redux/actions/courseBenefitActions';
...
export default function CourseBenefitScreen() {
    // permission get
    const {
        permission,
        setPermission,
    ...
    const courseBenefitList = useSelector(state => state.courseBenefitList);
    //eslint-disable-next-line
    const { courseBenefits, loading, error } = courseBenefitList;
    const courseBenefitSave = useSelector(state => state.courseBenefitSave);
    //eslint-disable-next-line
    const { loading: loadingSave, success: successSave, error: errorSave } = courseBenefitSave;
    const courseBenefitDelete = useSelector(state => state.courseBenefitDelete);
    //eslint-disable-next-line
    const { loading: loadingDelete, success: successDelete, error: errorDelete } = courseBenefitDelete;
    ...
    } = useTable(courseBenefits, headCells, filterFn);

    const dispatch = useDispatch();

    // add/update promise
    const saveItem = (item) => new Promise((resolve, reject) => {
        dispatch(saveCourseBenefit(item));
        resolve();
    })

    // delete promise
    const deleteItem = (id) => new Promise((resolve, reject) => {
        dispatch(deleteCourseBenefit(id));
        resolve();
    })
        ...
        <>
            {
                (loadingRoleResource || loading || loadingSoftwares || loadingTrainingTypes || loadingSave || loadingDelete) ? <Loading /> :
                    (
                        ...
                        <>
                            <PageTitle title="Course Benefits" />

                            <Grid container spacing={4}>
                                <Grid item xs={12}>
                                    <Widget
                                        title="Course Benefit List Table"
                                        upperTitle
                                        noBodyPadding
