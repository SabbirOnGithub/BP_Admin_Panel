...
// redux actions
import { listTrainingTypes, saveTrainingType, deleteTrainingType } from '../../../redux/actions/trainingTypeActions';
...
const headCells = [
    ...
    { id: 'paymentDiscount', label: 'Payment Discount' },
...
export default function TrainingTypeScreen() {
    ...
    const { trainingTypes, loading, error } = trainingTypeList;
    const trainingTypeSave = useSelector(state => state.trainingTypeSave);
    //eslint-disable-next-line
    const { loading: loadingSave, success: successSave, error: errorSave } = trainingTypeSave;
    const trainingTypeDelete = useSelector(state => state.trainingTypeDelete);
    //eslint-disable-next-line
    const { loading: loadingDelete, success: successDelete, error: errorDelete } = trainingTypeDelete;
    ...
    } = useTable(trainingTypes, headCells, filterFn);

    const dispatch = useDispatch();

    // add/update promise
    const saveItem = (item) => new Promise((resolve, reject) => {
        dispatch(saveTrainingType(item));
        resolve();
    })
        ...
        <>
            {
                (loadingRoleResource || loading || loadingSave || loadingDelete) ? <Loading /> :
                    (
                        ...
                        <>
                            <PageTitle title="Training Types" />

                            <Grid container spacing={4}>
                                <Grid item xs={12}>
                                    <Widget
                                        title="Training Type List Table"
                                        upperTitle
                                        noBodyPadding
                                                ...
                                                <TableBody>
                                                    {
                                                        ...
                                                        (<TableRow key={item?.id}>
                                                            ...
                                                            <TableCell>{item?.paymentDiscount}</TableCell>
