...
import TrainingOverviewForm from "./TrainingOverviewForm";
...
// redux actions
import { deleteTrainingOverview, listTrainingOverview, saveTrainingOverview } from '../../../redux/actions/trainingOverviewActions';
...
export default function TrainingOverviewScreen() {
    ...
    const trainingOverviewList = useSelector(state => state.trainingOverviewList)
    //eslint-disable-next-line
    const { trainingOverviews, loading, error } = trainingOverviewList;
    //eslint-disable-next-line
    const trainingOverviewSave = useSelector(state => state.trainingOverviewSave);
    //eslint-disable-next-line
    const { loading: loadingSave, success: successSave, error: errorSave } = trainingOverviewSave;
    const trainingOverviewDelete = useSelector(state => state.trainingOverviewDelete);
    //eslint-disable-next-line
    const { loading: loadingDelete, success: successDelete, error: errorDelete } = trainingOverviewDelete;
    ...
    } = useTable(trainingOverviews, headCells, filterFn);

    const dispatch = useDispatch();

    // add/update promise
    const saveItem = (item, id) => new Promise((resolve, reject) => {
        dispatch(saveTrainingOverview(item, id));
        resolve();
    })

    // delete promise
    const deleteItem = (id) => new Promise((resolve, reject) => {
        dispatch(deleteTrainingOverview(id));
        resolve();
    })
        ...
        <div>
            {
                (loadingRoleResource || loading || loadingSave || loadingDelete) ? <Loading /> :
                    ...
                    (trainingOverviews.length >= 0 &&
                        <>
                            <PageTitle title="Training Overview" />

                            <Grid container spacing={4}>
                                <Grid item xs={12}>
                                    <Widget
                                        title="Training Overview List Table"
                                        upperTitle
                                        noBodyPadding
                                    ...
                                    >
                                        ...
                                        <Popup
                                            title="Training Overview Form"
                                            openPopup={openPopup}
                                            setOpenPopup={setOpenPopup}
                                        >
                                            <TrainingOverviewForm
