...
import ConsultationOverviewForm from "./ConsultationOverviewForm";
...
// redux actions
import { deleteConsultationOverview, listConsultationOverview, saveConsultationOverview } from '../../../redux/actions/consultationOverviewActions';
...
export default function ConsultationOverviewScreen() {
    // permission get
    const {
        permission,
        setPermission,
    ...
    const consultationOverviewList = useSelector(state => state.consultationOverviewList)
    //eslint-disable-next-line
    const { consultationOverviews, loading, error } = consultationOverviewList;
    //eslint-disable-next-line
    const consultationOverviewSave = useSelector(state => state.consultationOverviewSave);
    //eslint-disable-next-line
    const { loading: loadingSave, success: successSave, error: errorSave } = consultationOverviewSave;
    const consultationOverviewDelete = useSelector(state => state.consultationOverviewDelete);
    //eslint-disable-next-line
    const { loading: loadingDelete, success: successDelete, error: errorDelete } = consultationOverviewDelete;
    ...
    } = useTable(consultationOverviews, headCells, filterFn);

    const dispatch = useDispatch();

    // add/update promise
    const saveItem = (item, id) => new Promise((resolve, reject) => {
        dispatch(saveConsultationOverview(item, id));
        resolve();
    })

    // delete promise
    const deleteItem = (id) => new Promise((resolve, reject) => {
        dispatch(deleteConsultationOverview(id));
        resolve();
    })
    ...
    useEffect(() => {
        try {
            ...
            if (recievedPermission?.readOperation) {
                ...
                dispatch(listConsultationOverview());
            }
        ...
        <div>
            {
                (loadingRoleResource || loading || loadingSave || loadingDelete) ? <Loading /> :
                    ...
                    (consultationOverviews.length >= 0 &&
                        <>
                            <PageTitle title="Consultation Overview" />

                            <Grid container spacing={4}>
                                <Grid item xs={12}>
                                    <Widget
                                        title="Consultation Overview List Table"
                                        upperTitle
                                        noBodyPadding
                                    ...
                                    >
                                        ...
                                        <Popup
                                            title="Consultation Overview Form"
                                            openPopup={openPopup}
                                            setOpenPopup={setOpenPopup}
                                        >
                                            <ConsultationOverviewForm
